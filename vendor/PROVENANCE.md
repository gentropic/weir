...
| `accountable-client.js` | `../accountable` repo `client/accountable-client.js` | @gcu/accountable 0.9.0 (gentropic/accountable@c031b8a) | CC0-1.0 | Page-side fetch broker. Exports `gcuFetch`, `hasAccountable`, `accountableVersion`, `clearAccountableCache`, `openAccountableSetup`. **Re-vendored 2026-06-27 after the pre-publish review: `gcuFetch` now supports `{ stream: true }` (ReadableStream Response) + `{ receipt: true }` (`res.receipt` with a verifiable sha256), and a brokered DENIAL (`accountable: …`) now SURFACES instead of silently falling back to a CORS-doomed direct fetch (weir's banner/`openAccountableSetup` is the recovery). Streaming/receipts not yet consumed weir-side.** **Re-vendored 2026-06-26 for the `@gcu/bridge` → `@gcu/accountable` rename (vN.0 mechanical rename: wire `gcu-bridge-*`→`gcu-acc-*`, marker `dataset.gcuAcc`, header `x-gcu-acc-cache`); access model unchanged (`<all_urls>` stays — `optional_host_permissions`/RO-RW is vN.1).** Probed non-blockingly for status; the poller's transport. |
