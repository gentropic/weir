...
| `accountable-client.js` | `../accountable` repo `client/accountable-client.js` | @gcu/accountable 0.8.0 (gentropic/accountable@5fc5854) | CC0-1.0 | Page-side fetch broker. Exports `gcuFetch`, `hasAccountable`, `accountableVersion`, `clearAccountableCache`, `openAccountableSetup`. **Re-vendored 2026-06-27 for streaming: `gcuFetch(url, { stream: true })` returns a ReadableStream-backed Response (chunked over a Port) — for big bodies / the documents-PDF path; not yet consumed weir-side. Earlier: `openAccountableSetup` (grant-summon).** **Re-vendored 2026-06-26 for the `@gcu/bridge` → `@gcu/accountable` rename (vN.0 mechanical rename: wire `gcu-bridge-*`→`gcu-acc-*`, marker `dataset.gcuAcc`, header `x-gcu-acc-cache`); access model unchanged (`<all_urls>` stays — `optional_host_permissions`/RO-RW is vN.1).** Probed non-blockingly for status; the poller's transport. |
