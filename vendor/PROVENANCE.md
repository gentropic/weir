...
| `accountable-client.js` | `../accountable` repo `client/accountable-client.js` | @gcu/accountable 0.4.0 (gentropic/accountable@ee1f7fb) | CC0-1.0 | Page-side fetch broker. Exports `gcuFetch`, `hasAccountable`, `accountableVersion`, `clearAccountableCache`. **Re-vendored 2026-06-26 for the `@gcu/bridge` → `@gcu/accountable` rename (vN.0 mechanical rename: wire `gcu-bridge-*`→`gcu-acc-*`, marker `dataset.gcuAcc`, header `x-gcu-acc-cache`); access model unchanged (`<all_urls>` stays — `optional_host_permissions`/RO-RW is vN.1).** Probed non-blockingly for status; the poller's transport. |
