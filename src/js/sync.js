...
class SyncEngine {
  ...
  async push() {
    ...
    if (be && typeof be.writeFiles === 'function' && toUpload.length) {
      // Fast path: commit in chunks via the backend's batch upload (Dropbox finish_batch) — far
      // fewer write-lock conflicts than per-file uploads, and it handles 429/Retry-After itself.
      const CHUNK = 900;   // Dropbox finish_batch caps at 1000 entries/call
      for (let i = 0; i < toUpload.length; i += CHUNK) {
        const chunk = toUpload.slice(i, i + CHUNK);
        const files = await Promise.all(chunk.map(async ({ p }) => ({ path: p, content: await this.local.readFile(p, 'bytes') })));
        await be.writeFiles(files);
        for (const { p, sig } of chunk) { man.files[p] = sig; pushed++; if (paths.length < 100) paths.push(p); }
        await this._saveManifest();   // checkpoint per batch → resumable
        this._progress('push', pushed, toUpload.length);
      }
    } else {
