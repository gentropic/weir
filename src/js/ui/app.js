  ...
  async _runCatalog(todo) {
    ...
    try {
      for (const it of todo) {
        if (job.cancel) break;
        this._catStatus(`cataloging ${n + 1}/${todo.length}…  ${ok} done${fails ? `, ${fails} failed` : ''} · click to stop`);
        const r = await this.catalogItem(it.id);
        n++;
        if (r) { ok++; streak = 0; } else if (++streak >= 8) {
          // 8 misses in a row → the LLM or bridge is down; stop instead of
