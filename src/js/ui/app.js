...
export class App {
  constructor({ store, poller, router, adapters, faviconFetcher }) {
    ...
    this._notePanes = new Map(); // rails tabId → NotePane (the graduated, multi-instance note editor)
    this._noteN = 0;             // monotonic note-tab counter
    this._lastNoteStackId = null;// MRU notes stack, for coalescing new notes (see _resolveNotesStack)
  ...
  mount() {
    ...
    // (note editor is now a rails pane — each NotePane wires its own save/modes/preview-sync; no modal buttons)
  ...
  itemMenu(id, x, y) {
    ...
    showMenu(x, y, [
      ...
      { label: '✎ Note…', onClick: () => this.annotateItem(id) },
  ...
  onKey(e) {
    ...
    // Cmd/Ctrl-S saves the focused note pane (the editor is a rails pane now, not a modal).
    const npEl = (e.target.closest && e.target.closest('.note-pane')) || null;
    if (npEl && (e.metaKey || e.ctrlKey) && (e.key === 's' || e.key === 'S')) { e.preventDefault(); npEl._notePane?.save(); return; }
    ...
    // NOTE: no modal key-swallow anymore. The note editor lives BESIDE the stream, so
    // j/k/r/s/n must keep working when the editor isn't focused. The "are they typing?"
    // guard below (INPUT/TEXTAREA/contenteditable/.cm-editor) already stops single-key
    // shortcuts from firing while the editor or its fields have focus.
    ...
    switch (e.key) {
      ...
      case 'n': {   // note: about the selected item (annotation-in-context), else a blank note
        e.preventDefault();
        const sel = this.selectedId ? this.store.getItem(this.selectedId) : null;
        if (sel && sel.feed_id !== 'stacks') this.annotateItem(this.selectedId);
        else this.openNotePane({});
        break;
      }
  ...
  _initWorkspace() {
    ...
    // Keep the "solo" class (hides the tab strip so the single-pane default reads like
    // before) in sync as panes open/close, re-measuring when it flips. ws-solo is
    // pre-set before createRails, so the initial solo state needs no toggle here.
    this.rails.on('layout:change', () => this._reflectSoloRelayout());
  }
  ...
  _renderPanel(tab) {
    ...
    if (tab.kind === 'note') {
      let pane = this._notePanes.get(tab.id);
      if (!pane) { pane = new NotePane(this, tab._noteOpts || {}); pane.tabId = tab.id; this._notePanes.set(tab.id, pane); }
      requestAnimationFrame(() => pane.init());   // mount CM6 + load AFTER it's in the DOM
      return pane.el;
    }
  ...
  // ── note editor — now a rails PANE (per-instance NotePane), not a modal ──
  // Back-compat entry: existing call sites (stacks ＋note, palette, context menus,
  // the `n` key) call openNoteEditor(id). id present = edit that note; absent = new.
  openNoteEditor(id) { this.openNotePane({ noteId: id || null }); }
  // Annotation-in-context: open a note bound to a stream item (W3C target = item id).
  annotateItem(id) {
    const it = this.store.getItem(id); if (!it) return;
    this.openNotePane({ target: { id: it.id, title: it.title || it.path || it.id, type: it.type || 'item' } });
  }
  // Open (or focus) a note pane. Coalesces: an already-open note for the same id is
  // re-focused; otherwise a new pane lands in the live notes stack (MRU), or summons
  // one if none is open. Mirrors the prototype's placement policy.
  openNotePane({ noteId = null, target = null } = {}) {
    if (!this.rails) return;
    if (noteId) {   // already open? focus it instead of duplicating
      for (const [tid, p] of this._notePanes) if (p.noteId === noteId) { this.rails.activateTab(tid); p.focus(); return; }
    }
    const tabId = 'note-' + (noteId || 'new') + '-' + (++this._noteN);
    const label = target ? this._shortTitle(target.title)
      : (noteId ? this._shortTitle(this.store.getItem(noteId)?.title || 'note') : 'new note');
    const tab = { id: tabId, kind: 'note', title: '✎ ' + label, closeable: true, _noteOpts: { noteId, target } };
    const stackId = this._resolveNotesStack();
    if (stackId) this.rails.addTab(tab, { to: 'stack', stackId });
    else this.rails.addTab(tab, { to: 'new-rail' });
    this._lastNoteStackId = this._findStackOfTab(tabId);
    this._reflectSoloRelayout();   // solo → multi: the strip appears; re-measure slots
  }
  // remove a note pane's bookkeeping when its tab closes (wired via onPanelDestroy).
  _disposeNotePane(tabId) {
    const p = this._notePanes.get(tabId);
    if (p) { p.dispose(); this._notePanes.delete(tabId); }
    this._reflectSoloRelayout();
  }
  _notePaneOf(el) { const host = el?.closest?.('.note-pane'); return host ? host._notePane : null; }
  _shortTitle(t) { t = String(t || 'note'); return t.length > 26 ? t.slice(0, 24) + '…' : t; }

  // ── notes-stack placement (consumer-owned policy; see proto/rails-notes.html) ──
  _stackById(id) {
    if (!id || !this.rails) return null;
    for (const r of this.rails.state.rails) for (const s of r.stacks) if (s.id === id) return s;
    return null;
  }
  _findStackOfTab(tabId) {
    if (!this.rails) return null;
    for (const r of this.rails.state.rails) for (const s of r.stacks)
      if (s.tabs.some((t) => t.id === tabId)) return s.id;
    return null;
  }
  _resolveNotesStack() {
    const isNote = (s) => s.tabs.some((t) => t.kind === 'note');
    const mru = this._stackById(this._lastNoteStackId);   // where you last looked, if it survives
    if (mru && isNote(mru)) return mru.id;
    let any = null;                                        // else any stack holding a note
    for (const r of this.rails.state.rails) for (const s of r.stacks)
      if (isNote(s)) {
        const active = s.tabs.find((t) => t.id === s.active);
        if (active?.kind === 'note') return s.id;          // prefer one already showing a note
        any = any || s.id;
      }
    return any;                                            // null → none open → caller summons a rail
  }
  // Toggle the single-pane "solo" class (hides the tab strip) and re-measure rails'
  // slots when it changes, so panels fill correctly as the strip shows/hides.
  _reflectSoloRelayout() {
    const host = document.getElementById('workspace'); if (!host || !this.rails) return;
    const rs = this.rails.state.rails;
    const solo = rs.length === 1 && rs[0].stacks.length === 1
      && rs[0].stacks[0].tabs.length === 1 && rs[0].stacks[0].tabs[0].kind === 'stream';
    if (host.classList.contains('ws-solo') !== solo) {
      host.classList.toggle('ws-solo', solo);
      this.rails.render();   // strip shown/hidden → reposition cached panels (does NOT rebuild them)
    }
  }
...
// notes edit side-by-side with the stream and survive every drag (rails never
// reparents a panel). Annotation-in-context: a pane may be bound to a stream item via
// `target` (the W3C Web-Annotation target → persisted as the note's `target` frontmatter).
class NotePane {
  constructor(app, { noteId = null, target = null } = {}) {
    this.app = app;
    ...
    this._inited = false;
    this.el = this._build();
    this.el._notePane = this;    // back-ref for Cmd-S / focus lookups
  }
  _build() {
    const t = this.target;
    ...
    el.innerHTML =
      `<div class="note-head se-head">`
      + (t ? `<span class="note-tgt" title="this note annotates a stream item">✎ <span class="np-pill" style="background:var(${NOTE_ACCENT[t.type] || '--sw-teal'})">${escapeHtml(t.type)}</span> <span class="np-tgt-title">${escapeHtml(t.title)}</span></span>` : '')
      + `<input class="field se-title np-title" type="text" placeholder="note title…" autocomplete="off" spellcheck="false">`
      + `<span class="se-path np-path"></span>`
      + `<span class="se-modes">`
      + `<button class="btn-link se-mode np-mode" data-m="edit" type="button" title="Edit only">✎ edit</button>`
      + `<button class="btn-link se-mode np-mode" data-m="split" type="button" title="Edit + preview">▥ split</button>`
      + `<button class="btn-link se-mode np-mode" data-m="preview" type="button" title="Preview only">▦ preview</button>`
      + `</span></div>`
      + `<div class="se-body np-body"><div class="se-edit np-edit"></div><div class="se-preview icontent np-preview"></div></div>`
      + `<div class="se-foot np-foot">`
      + `<input class="field se-tags np-tags" type="text" placeholder="tags, comma-separated…" autocomplete="off" spellcheck="false">`
      + `<span class="hint np-status"></span>`
      + `<button class="btn btn-primary np-save" type="button">Save</button>`
      + `</div>`;
  ...
  ensureEditor() {
    ...
    const host = this._q('.np-edit');
    const CM = window.CM6;
    const extra = [];
    if (CM && CM.autocompletion) {   // [[ holdings autocomplete — same source as the old modal
      extra.push(CM.autocompletion({ override: [(ctx) => this.app._wikiCompletions(ctx)], activateOnTyping: true, icons: false }));
    ...
    this.editor = this.app._mountCm(host, (CM && CM.markdown) ? CM.markdown() : null, () => this._scheduleSync(), extra);
    return this.editor;
  }
  _scheduleSync() { clearTimeout(this._pvTimer); this._pvTimer = setTimeout(() => this.syncPreview(), 150); }
  getValue() { return this.editor ? this.editor.get() : ''; }
  setValue(t) { this.ensureEditor().set(t || ''); }
  setMode(mode) {
    this.mode = mode; this.app._noteMode = mode;
    const body = this._q('.np-body'); if (body) body.className = `se-body np-body mode-${mode}`;
    this.el.querySelectorAll('.np-mode').forEach((b) => b.classList.toggle('active', b.dataset.m === mode));
    if (mode !== 'edit') this.syncPreview();
    if (mode !== 'preview' && this.editor) this.editor.measure();
  }
  syncPreview() {
    const pv = this._q('.np-preview'); if (!pv) return;
    pv.innerHTML = renderMarkdown(this.getValue()) || '<p class="se-empty">nothing to preview</p>';
  }
  async init() {
    if (this._inited) { this.editor?.measure?.(); return; }   // re-entry → just re-measure
    this._inited = true;
    this.ensureEditor();
    this.setMode(this.mode);
    if (this.noteId) {
      const it = this.app.store.getItem(this.noteId);
      if (it) {
        this._q('.np-title').value = it.title || '';
        this._q('.np-path').textContent = it.path || '';
        this._q('.np-tags').value = (it.tags || []).join(', ');
        const body = it.type === 'note' ? await this.app.stacks.readNote(it) : '';
        this.setValue(body);
      }
    } else {
      this._q('.np-path').textContent = this.target ? `inbox/ · note on “${this._shortT(this.target.title)}”` : 'inbox/ · new note';
    }
    this.syncPreview();
    this.focus();
  }
  _shortT(t) { t = String(t || ''); return t.length > 30 ? t.slice(0, 28) + '…' : t; }
  focus() { if (this.editor) this.editor.focus(); else this._q('.np-title')?.focus(); }
  async save() {
    const app = this.app; if (!app.stacks) return;
    const titleEl = this._q('.np-title'), tagsEl = this._q('.np-tags'), statusEl = this._q('.np-status');
    const md = this.getValue(); const title = titleEl.value.trim() || undefined;
    const tags = tagsEl.value.split(',').map((t) => t.trim().toLowerCase()).filter(Boolean);
    try {
      let rec;
      if (this.noteId) { const it = app.store.getItem(this.noteId); rec = await app.stacks.saveNote(it, md, { title, tags }); }
      else { rec = await app.stacks.writeNote({ title, markdown: md, tags, target: this.target ? this.target.id : undefined }); }
      await app.store.flush();
      this.noteId = rec.id; app._content.delete(rec.id);
