...
export class StacksStore {
  ...
  async scan() {
    ...
    const allRels = new Set(files.map((a) => this._rel(a)));
    for (const abs of files) {
      const rel = this._rel(abs);
      const base = this._basename(rel);
      if (isHidden(base) || isSidecar(rel)) continue;
      // note vs file: by extension, BUT a `.meta.json` sidecar marks a deliberately-
      // ingested file (even a .md/.txt) — so it isn't re-read as a note on rescan.
      const asNote = isNotePath(rel) && !allRels.has(`${rel}.meta.json`);
      const entry = asNote ? await this._scanNote(abs, rel, res) : await this._scanFile(abs, rel, res);
      if (!entry) continue;
      present.add(entry.uid);
      this.store.syncStacksEntry(entry);
      asNote ? res.notes++ : res.files++;
    }
