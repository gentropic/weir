...
export function buildWeirTools({ store, cardFacets, ensureCards, app } = {}) {
  ...
  const DEFAULT_WEIGHTS = { curated: 2.5, neutral: 1.0, firehose: 0.6, facet: 0.2 };
  ...
  async function search(input = {}) {
    ...
    if (input.weights && typeof input.weights === 'object') {
      ...
      weights = { curated: clamp(input.weights.curated, DEFAULT_WEIGHTS.curated), neutral: clamp(input.weights.neutral, DEFAULT_WEIGHTS.neutral), firehose: clamp(input.weights.firehose, DEFAULT_WEIGHTS.firehose), facet: clamp(input.weights.facet, DEFAULT_WEIGHTS.facet) };
    }
    if (idx && idx.ready) {
      const preds = [];
