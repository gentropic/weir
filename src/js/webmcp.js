...
export function buildWeirTools({ store, cardFacets, ensureCards, app } = {}) {
  ...
  async function search(input = {}) {
    ...
    // Unicode-aware tokenization (NFC) so an accented term (geoestatística) stays ONE token
    // and reaches vocab expansion — was splitting on the accent into geoestat+stica, which
    // broke the pt-BR→EN synonym bridge (SPEC-retrieval-tuning EVAL2).
    let qTerms = q.normalize('NFC').toLowerCase().split(/[^\p{L}\p{N}]+/u).filter((w) => w.length > 1);
  ...
  // Transport diagnostics for the numen fs channels. Per channel: connection state +
  // clientId, token presence, the folder-handle NAME it points at, what weir actually
  // READS from that folder's bridge.live (present? session, age, fresh<90s?), and the
  // folder's top-level entries. Splits a FOLDER MISMATCH (weir + the bridge on different
  // dirs → bridgeLive absent/stale though the bridge announces) from a STALE HANDSHAKE
  // (bridgeLive fresh but no clientId). Handles come from the app cache (Settings →
  // Connections populates them); a channel with hasHandle:false just needs that opened once.
  async function mcpDiag() {
    const wm = app && app.webmcp;
    const known = (wm && wm.knownChannels) ? wm.knownChannels() : [];
    const live = (wm && wm.channels) ? wm.channels() : [];
    const handles = (app && app._webmcpFsHandles) || {};
    const now = Date.now();
    const channels = [];
    for (const { id, identity } of known) {
      const ch = live.find((c) => c.id === id) || null;
      const row = {
        id, identity,
        state: ch ? ch.state : 'not-connected',
      ...
      if (h) {
        ...
        try {
          ...
          row.bridgeLive = { present: true, session: b.session, ageMs: now - b.ts, fresh: (now - b.ts) < 90000 };
        } catch { row.bridgeLive = { present: false }; }
